...
one sig CohReq extends Location { }
one sig CohResp extends Location { }
...
fact {
	...
	CacheableRead->{ Fetch + Execute + ReorderBuffer + PermissionCheck + Complete } +				
   	...
   	CacheableWrite->{  Fetch + Execute + ReorderBuffer + PermissionCheck + Complete + CohReq + CohResp } + 
...
fact { NodeRel in 
	...
	CacheableRead->{  Fetch + Execute + ReorderBuffer + PermissionCheck + Commit + CohReq + CohResp + ViCLCreate + ViCLExpire + Complete } +
   	NonCacheableRead->{  Fetch + Execute + ReorderBuffer + PermissionCheck + Commit + Complete } +				
	CacheableWrite->{  Fetch + Execute + ReorderBuffer + PermissionCheck + Commit + StoreBuffer + CohReq + CohResp + ViCLCreate + ViCLExpire + MainMemory + Complete } +											
...
fact dep_vicl_source {
	all disj e, e': Event | (HasDependency[e, e'] and NodeExists[e, ViCLCreate] and NodeExists[e', CohReq] ) <=> EdgeExists[e, ViCLCreate, e', CohReq, udep]
}

//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// threading and process scheduling
// assign threads to processes and to cores
...
fact squash_spec_window_one { all e : Event | all b : MispredictedBranch | (b->e in po) =>
	(
		...
		(NodeExists[e, CohReq] => EdgeExists[e, CohReq, b, Execute, usquash]) and
...
fact coh_permissions_cohreq_pc { all e, e': Event | (EdgeExists[e, ReorderBuffer, e', PermissionCheck, usquash]) => (NodeExists[e, CohReq] => EdgeExists[e, CohReq, e', PermissionCheck, usquash])}
fact coh_permissions_rob_pc { all e, e': Event | (EdgeExists[e, CohReq, e', PermissionCheck, usquash]) => (EdgeExists[e, ReorderBuffer, e', PermissionCheck, usquash])}
...
fact { all e, e' : Event | all l, l' : Location | EdgeExists[e, l, e', l', usquash] =>
	(not SameEvent[e, e'] and (l in ReorderBuffer or l in CohReq or l in ViCLCreate) and not NodeExists[e, Commit]) and
	( (e' in Branch and l' = Execute) or ((IsIllegalRead[e'] or IsIllegalWrite[e']) and l' = PermissionCheck))
}

//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// finer modeling granularity for coherence
fact coh_transitive { transitive[coh] }			
fact co_in_coh { co in coh }
fact co_in_po_loc { po_loc in coh }
fact coh_in_same_addr { coh in (address.map).~(address.map) }
fact coh_total { all a : Address | total[coh, a.~(address.map) & NodeRel.CohResp] }
fact draw_ucoh_inter { all disj e, e' : Event 	|  (e->e' in coh and NodeExists[e, CohResp] and NodeExists[e', CohResp]) <=> EdgeExists[e, CohResp, e', CohResp, ucoh_inter] } 

fact CohReqRespPairs { all e : Event | NodeExists[e, CohReq] <=> NodeExists[e, CohResp] }
fact CohReqViCLs { all r : Read | NodeExists[r, ViCLCreate] <=> NodeExists[r, CohReq] }

fact reads_ucoh_intra {
	all r : Read | NodeExists[r, CohReq] <=> 
		(EdgeExists[r, Fetch, r, CohReq, ucoh_intra] and
		EdgeExists[r, CohReq, r, CohResp, ucoh_intra] and
		EdgeExists[r, CohResp, r, ViCLCreate, ucoh_intra])
}

fact single_writer {
	all e, e': Event |	(IsAnyWrite[e']  and not SameCore[e, e'] and e->e' in coh and NodeExists[e, ViCLExpire] and NodeExists[e', CohResp] ) <=>
							EdgeExists[e, ViCLExpire, e', CohResp, ucoh_inter]
}

fact bound_ucoh_inter { ucoh_inter in
	Event->CohResp->Event->CohResp +
	Event->ViCLExpire->Write->CohResp
}

fact bound_ucoh_intra { ucoh_intra in
	Read->Fetch->Read->CohReq +
	Read->CohReq->Read->CohResp +
	Read->CohResp->Read->ViCLCreate
}

fact {
        all r, r' : Read |
        (
			(EdgeExists[r, Fetch, r', CohReq, ucoh_intra] or
			EdgeExists[r, CohReq, r', CohResp, ucoh_intra] or
			EdgeExists[r, CohResp, r', ViCLCreate, ucoh_intra])
		) => SameEvent[r, r']
}
...
fact bound_udep { udep in 
	...
	Read->ViCLCreate->MemoryEvent->CohReq
}
...
fact explicit_evictions {
	all disj e, e' : CacheableEvent |
			...
			(no e'' : CacheableEvent | EdgeExists[e, ViCLExpire, e'', CohResp, ucoh_inter] and not SameEvent[e'', e'] and EdgeExists[e'', CohResp, e', CohResp, ucoh_inter]) and
...
pred prime_probe {
	...
	SameVirtualAddress[a, a'] and
	(IsAnyRead[a] and SameSourcingWrite[a, a'] or IsAnyWrite[a] and (a->a' in rf))  and
	ProgramOrder[a, a'] and
	...
	// attacker will not avoid its own exploit
	not (some e: AttackerEvent |
  		not SameCore[e, a] and
       SamePhysicalAddress[e, a] and
		NodeExists[e, Commit] and
		EdgeExists[a, ViCLExpire, e, CohResp, ucoh_inter]) and

	// exploit starts at the prime phase and ends after the probe stage
  	not (some e: AttackerEvent | ProgramOrder[e, a]) and
  	not (some e: AttackerEvent | ProgramOrder[a', e]) and
  	not (some e: Event | EdgeExists[e, Complete, a, Fetch, uhb_proc]) and
  	not (some e: Event | EdgeExists[a', Complete, e, Fetch, uhb_proc])
...
run test_flush_reload {
	...
	#dep = 0 and
	ucheck
} for 65 but 1 Core, 2 Process, 1 CacheIndexL1, 1 Cacheability, 2 Outcome, 4 Address, 2 PhysicalAddress, 2 VirtualAddress, 12 Location, 4 Event, 38 Node
...
run test_meltdown {
	...
	ucheck
} for 77 but 1 Core, 2 Process, 2 CacheIndexL1, 1 Cacheability, 2 Outcome, 6 Address, 3 PhysicalAddress, 3 VirtualAddress, 12 Location, 5 Event, 46 Node
...
run test_spectre {
	...
	NumProcessThreads[0, Victim] and
	ucheck
} for 85 but 1 Core, 2 Process, 2 CacheIndexL1, 1 Cacheability, 2 Outcome, 6 Address, 3 PhysicalAddress, 3 VirtualAddress, 12 Location, 6 Event, 52 Node
...
run test_prime_probe {
	...
	NumProcessThreads[1, Victim] and
	ucheck
} for 60 but 1 Core, 2 Process, 1 CacheIndexL1, 1 Cacheability, 2 Outcome, 4 Address, 2 PhysicalAddress, 2 VirtualAddress, 12 Location, 3 Event, 34 Node

run test_meltdown_prime {
	no_shared_mem and
	prime_probe and 
	#dep = 1 and
	(all disj e, e' : Event | HasDependency[e, e'] => (Victim in PhysicalAddress[e].region and Attacker in PhysicalAddress[e'].region)) and
	NumProcessThreads[2, Attacker] and
	NumProcessThreads[0, Victim] and
	ucheck
} for 67 but 2 Core, 2 Process, 2 CacheIndexL1, 1 Cacheability, 2 Outcome, 4 Address, 2 PhysicalAddress, 2 VirtualAddress, 12 Location, 4 Event, 38 Node

run test_spectre_prime {
	no_shared_mem and
 	prime_probe and 
 	meltdown_fix and
 	#dep = 1 and
	(all disj e, e' : Event | HasDependency[e, e'] => (Victim in PhysicalAddress[e].region and Attacker in PhysicalAddress[e'].region)) and
 	...
 	NumProcessThreads[0, Victim] and
 	ucheck
} for 75 but 2 Core, 2 Process, 2 CacheIndexL1, 1 Cacheability, 2 Outcome, 4 Address, 2 PhysicalAddress, 2 VirtualAddress, 12 Location, 5 Event, 44 Node
